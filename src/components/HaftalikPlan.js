...
export default function HaftalikPlan({ haftalikPlan, tarifler, menuler, planTemizle, plandanOgeSil, getGunlukTopluMalzemeler }) {
  ...
  // YENİ: Günlük detay ekranını açmak için state
  ...
  // YENİ: GÜNLÜK DETAY EKRANI (Menü detayının aynısı)
      ...
      <div className="animate-in fade-in duration-300">
        <div className="print:hidden">
          <div className="flex justify-between items-center mb-6">
            <button onClick={() => setDetayGosterilenGun(null)} className="flex items-center text-orange-800 hover:text-orange-600 font-bold">
            ...
            <button onClick={() => window.print()} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl font-bold flex items-center shadow-sm text-sm">
              <Printer size={18} className="mr-2" /> PDF / Mutfak Çıktısı Al
            </button>
          </div>
        ...
        {/* GÜNLÜK DETAY YAZDIRMA (PDF) ŞABLONU */}
    ...
    <div>
      <div className="animate-in fade-in duration-300 mb-12 print:hidden">
              ...
              <div key={gun.isoStr} className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 flex flex-col justify-between">
                <div>
                  <div className="border-b pb-2 mb-3 flex justify-between items-start">
                    ...
                    {planKaydi && (
                      <button onClick={() => planTemizle(gun.isoStr)} className="text-slate-400 hover:text-red-600 bg-slate-50 hover:bg-red-50 p-1.5 rounded-lg transition-colors" title="Tüm Günü Temizle"><Trash2 size={16} /></button>
                    )}
                  </div>
                      ...
                      {gununMenuleri.map((menu, idx) => (
                        <div key={`menu-${idx}`} className="mb-3 bg-orange-50/70 p-2 rounded-xl border border-orange-100 shadow-sm">
                          <div className="flex justify-between items-center mb-2">
                            ...
                            <button onClick={() => plandanOgeSil(gun.isoStr, 'menu', menu)} className="text-red-400 hover:text-red-600 hover:bg-red-50 p-1 rounded transition-colors"><X size={14}/></button>
                          </div>
                          ...
                          <div className="space-y-1.5">
                            {ekstraTarifler.map(t => (
                              <div key={t.id} className="bg-white p-1.5 rounded-lg border border-blue-50 flex justify-between items-center shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
                                ...
                                <button onClick={() => plandanOgeSil(gun.isoStr, 'tarif', t)} className="text-red-400 hover:text-red-600 hover:bg-red-50 p-1 rounded transition-colors shrink-0"><X size={14}/></button>
                              </div>
                            ))}
                          </div>
                        </div>
                ...
                {/* YENİ: GÜNÜN DETAYINI GÖR BUTONU */}
                {planKaydi && (
                  <button 
                    onClick={() => setDetayGosterilenGun(gun.isoStr)} 
                    className="w-full mt-3 bg-blue-50 hover:bg-blue-100 text-blue-700 p-2 rounded-lg text-xs font-bold flex items-center justify-center transition-colors border border-blue-100"
                  >
                    Günün Detayını Gör →
                  </button>
                )}
        ...
        <div className="bg-orange-50 p-6 rounded-2xl border border-orange-200 shadow-sm">
          ...
          {haftalikAlisveris.length === 0 ? (
            <p className="text-sm text-slate-500 italic">Bu hafta için henüz planlanmış bir menü bulunmuyor.</p>
          ) : (
