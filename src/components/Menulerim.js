...
export default function Menulerim({ 
  ...
  const [planModaliIcinMenu, setPlanModaliIcinMenu] = useState(null);
  ...
  const [hizliPrintMenu, setHizliPrintMenu] = useState(null);

  useEffect(() => {
    if (hizliPrintMenu) {
      setTimeout(() => { window.print(); setHizliPrintMenu(null); }, 200);
    }
  }, [hizliPrintMenu]);

  const takvimeIsle = (e) => {
    e.preventDefault();
    if (!planTarihSecildi || !planModaliIcinMenu) return;
  ...
  if (detayMenu || hizliPrintMenu) {
    ...
    return (
      <div className="animate-in fade-in duration-300">
        ...
        <div className={`${hizliPrintMenu ? 'block print:block w-full bg-white text-black p-4 fixed inset-0 z-[300] overflow-auto' : 'hidden print:block print:w-full print:bg-white print:text-black print:p-4'}`}>
          <div className="text-center border-b-2 border-black pb-4 mb-6">
            <h1 className="text-3xl font-extrabold uppercase tracking-wider mb-2">Menü Programı</h1>
            <p className="text-lg font-medium text-gray-700">Menü: {islemGorenMenu.ad}</p>
          </div>
          <div className="mb-8">
  ...
  return (
    <div className="animate-in fade-in duration-300">
      ...
      {genelMenuArama.trim() !== '' ? (
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden mb-8">
          {renderMenulerListesi(genelFiltrelenmisMenuler, false)}
        </div>
      ) : (
